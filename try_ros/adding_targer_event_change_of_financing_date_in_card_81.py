...
try:
   ...
   #Меняем дату в карточке меропиятия
    ...
    finansMon_combo= browser.find_element_by_id("comboFinMonthText")
    finansMon_combo.click()
    time.sleep(3)
    feb_mon = browser.find_element_by_id("FinMon2")
    feb_mon.click()
    time.sleep(3)
    finansMon_combo.click()
    time.sleep(3)
    ...
    assert notif_text=="Месяц начала получения эффекта не может быть меньше месяца начала финансирования","Неверный текст"
